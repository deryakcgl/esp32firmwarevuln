#!/usr/bin/env python3
"""Batch create ground truth from GitHub vulnerability reports"""
...
from scripts.create_ground_truth_from_github import load_github_vulnerability_reports, match_vulnerability_to_functions
from esp32_firmguard.pipeline import FirmwareSecurityPipeline


def create_ground_truth_for_firmware(
    firmware_path: Path,
    config: dict,
    vulnerability_reports: list,
    output_dir: Path
):
    """Create GitHub-based ground truth for a single firmware"""
    try:
...
def main():
    parser = argparse.ArgumentParser(description="Batch create GitHub-based ground truth")
    ...
    if not vulnerability_reports:
        print("Warning: No vulnerability reports found.")
        return
    ...
    print(f"\nCreating GitHub-based ground truth for {len(firmware_files)} firmware...")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(create_ground_truth_for_firmware, fw, config, vulnerability_reports, output_dir): fw
    ...
    print("✅ This is REAL ground truth (not synthetic) from GitHub vulnerability reports!")
