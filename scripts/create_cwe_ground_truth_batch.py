#!/usr/bin/env python3
"""Batch create CWE-based ground truth for multiple firmware"""
...
def create_ground_truth_for_firmware(firmware_path: Path, config: dict, output_dir: Path):
    """Create CWE-based ground truth for a single firmware"""
    try:
        ...
        fw, feature_matrix, predictions, cwe_labels = pipeline.run_static_stage(
            str(firmware_path),
            source="cwe_ground_truth"
        )
        
        # CWE label varsa vulnerable (1), yoksa safe (0)
        ground_truth = {}
        for func_id in fw.functions.keys():
            ground_truth[func_id] = 1 if len(cwe_labels.get(func_id, [])) > 0 else 0
...
def main():
    parser = argparse.ArgumentParser(description="Batch create CWE-based ground truth")
    ...
    print(f"\nCreating CWE-based ground truth for {len(firmware_files)} firmware...")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(create_ground_truth_for_firmware, fw, config, output_dir): fw
